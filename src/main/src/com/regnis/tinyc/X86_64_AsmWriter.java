...
public abstract class X86_64_AsmWriter extends AsmWriter {
	...
	@Override
	protected void writeBranchRegReg(IRCompare.Op op, int leftReg, int rightReg, Type leftType, String targetLabel) throws IOException {
		final String command = getBranchJumpCommand(op, leftType);

		final String leftRegName = getRegName(leftReg, leftType);
		final String rightRegName = getRegName(rightReg, leftType);
		writeIndented("cmp " + leftRegName + ", " + rightRegName);
		writeIndented(command + " " + escapeLabel(targetLabel));
	}
	...
	protected void writeJump(String label) throws IOException {
		writeIndented("jmp " + escapeLabel(label));
	}
	...
	@NotNull
	private static String getBranchJumpCommand(@NotNull IRCompare.Op op, @NotNull Type type) {
