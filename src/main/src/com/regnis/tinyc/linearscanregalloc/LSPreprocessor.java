...
public final class LSPreprocessor {
	...
	public static Pair<IRVarInfos, List<IRInstruction>> process(@NotNull IRFunction function, @NotNull LSCallingConventionProvider callingConventionProvider, @Nullable X86Registers x86Registers, boolean hasStackSlotsForRegisterVars, @NotNull Type pointerIntType) {
		...
		final var resultLayer = new IRConverterResultLayer();
		...
		IRConverterLayer nextLayer = new LSPreprocessorCallingConventionLayer(function.varInfos(), tempVarFactory, callingConventionProvider, resultLayer);
		if (x86Registers != null) {
			nextLayer = new LSPreprocessorX86OperationsLayer(x86Registers, nextLayer);
		}

		nextLayer = new IRCachedVarConverterLayer(tempVarFactory, nextLayer);

		if (varToNewVar.size() > 0) {
			nextLayer = new LSPreprocessorReplaceVarLayer(varToNewVar, nextLayer);
		}

		IRConverterLayer.process(nextLayer, instructions);

		final IRVarInfos varInfos = tempVarFactory.createVarInfos();
		return new Pair<>(varInfos, resultLayer.instructions);
	}

	private static void storeRegisterArgsInVars(IRVarInfos varInfos, List<Integer> argRegisters, List<IRInstruction> instructions, IRConverterLayer layer) {
	...
	private static Map<IRVar, IRVar> storeRegisterArgsInNewVars(IRVarInfos varInfos, List<Integer> argRegisters, List<IRInstruction> instructions, IRLocalVarFactory tempVarFactory, IRConverterLayer layer) {
