...
final class LSIntervalFactory {
	...
	@NotNull Map<IRVar, LSInterval> getVarIntervals() {
		final Map<IRVar, LSInterval> varToInterval = new LinkedHashMap<>();
		for (LSInterval interval : varIntervals) {
			final LSInterval duplicate = varToInterval.put(interval.var(), interval);
			Utils.assertTrue(duplicate == null);
		}
...
//		System.out.println(getIndex() + ":\t" + instruction);
		...
		case IRBinary binary -> {
			...
			if (rightVar != null) {
				if (isX86) {
					...
					case ShiftLeft, ShiftRight -> expectRegister(rightVar, rcx);
					}
				}
				...
				if (isX86) {
					...
					case Div -> {
						...
						handleSource(left, live);
						getRegisterInterval(rcx).add(pos - 1, pos);
						return;
					}
					case Mod -> {
						// (rdx rax) % %reg -> rdx
		...
		case IRBranch branch -> {
			handleSource(branch.left(), live);
			final IRVar rightVar = branch.right().var();
			if (rightVar != null) {
				handleSource(rightVar, live);
			}
		}
		...
		case IRMove move -> {
			...
			if (sourceVar != null) {
				handleMove(sourceVar, target, live);
			}
			else {
				handleTarget(target, live);
			}
