...
public final class LSRegAlloc {
	...
	public static IRFunction process(@NotNull IRFunction function, @Nullable X86Registers x86Registers, boolean hasStackSlotsForRegisterVars, int registerCount, @NotNull LSCallingConventionProvider callingConventionProvider, @NotNull Type pointerIntType) {
		...
		final LSAlgorithmLogger logger = debug ? new LSAlgorithmLoggerImpl(blockBoundaries) : LSAlgorithmLogger.DUMMY;
		...
		if (varToInterval == null || containsSpills(varToInterval)) {
			...
			int registerCountWithSpillHelper = registerCount - 1;
			if (z8PointerIntType != null && (spillHelperRegister & 1) != 0) {
				spillHelperRegister--;
				registerCountWithSpillHelper--;
			}
			...
			varToInterval = LSAlgorithm.perform(intervalFactory.getVarIntervals(), intervalFactory.getFixedIntervals(), registerCountWithSpillHelper, z8PointerIntType, logger);
		}
	...
	private void processInstruction(IRInstruction instruction) {
		...
		case IRBinary binary -> {
			...
			if (rightVar != null) {
				right = new IRValue(sourceExpectReg(rightVar));
			}
			final IRVar target = targetExpectReg(binary.target());
			add(new IRBinary(target, binary.op(), left, right, binary.location()));
		}
		case IRBranch branch -> {
			final IRVar left = sourceExpectReg(branch.left());
			IRValue right = branch.right();
			final IRVar rightVar = right.var();
			if (rightVar != null) {
				right = new IRValue(sourceExpectReg(rightVar));
			}
			add(new IRBranch(branch.op(), left, right, branch.target(), branch.nextLabel()));
		}
		case IRCall call -> {
			...
			for (IRValue arg : call.args()) {
				...
				args.add(new IRValue(source(var)));
			}
		...
		case IRMove move -> {
			...
			if (sourceVar != null) {
				...
				final boolean sourceIsReg = sourceVar.scope() == VariableScope.register;
				final boolean targetIsReg = target.scope() == VariableScope.register;
				Utils.assertTrue(sourceIsReg || targetIsReg);

				boolean skip = sourceVar.equals(target);
				if (!skip
				    && sourceIsReg
				    && targetIsReg
				    && sourceVar.index() == target.index()) {
					skip = true;
				}
				if (!skip) {
					move(sourceVar, target, move.location());
				}
			}
			else {
				target = targetExpectReg(target);
				add(new IRMove(target, source.value(), move.location()));
			}
		}
	...
	private void processMoves() {
		final List<Pair<IRVar, IRVar>> registerToMemMoves = new ArrayList<>();
		final List<Pair<IRVar, IRVar>> registerToRegisterMoves = new ArrayList<>();
		final List<Pair<IRVar, IRVar>> memToRegisterMoves = new ArrayList<>();
		for (Map.Entry<IRVar, LSInterval> entry : varToInterval.entrySet()) {
			final IRVar var = entry.getKey();
			final LSInterval interval = entry.getValue();
			...
			if (transition != null) {
				...
				if (source.scope() != VariableScope.register) {
					...
					memToRegisterMoves.add(transition);
				}
				else if (target.scope() != VariableScope.register) {
					registerToMemMoves.add(transition);
				}
				else {
					registerToRegisterMoves.add(transition);
				}
			}
		}

		for (Pair<IRVar, IRVar> pair : registerToMemMoves) {
			move(pair.first(), pair.second());
		}
		if (registerToRegisterMoves.size() > 1) {
			throw new UnsupportedOperationException("Multiple register-to-register moves - maybe parallel moves?");
		}
		for (Pair<IRVar, IRVar> pair : registerToRegisterMoves) {
			move(pair.first(), pair.second());
		}
		for (Pair<IRVar, IRVar> pair : memToRegisterMoves) {
			move(pair.first(), pair.second());
		}
		...
		if (moves != null) {
			for (IRMove move : moves) {
				final IRVar sourceVar = move.source().var();
				Utils.assertTrue(sourceVar != null);
				move(sourceVar, move.target());
			}
		}
	}
	...
	private void add(IRInstruction instruction) {
		...
		if (debug) {
			System.out.println(pos + "\t" + instruction.toString(true));
		}
	}
