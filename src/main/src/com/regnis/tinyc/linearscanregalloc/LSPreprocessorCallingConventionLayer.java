 ...
 */
final class LSPreprocessorCallingConventionLayer extends IRConverterAbstractLayer {
	...
	public LSPreprocessorCallingConventionLayer(@NotNull IRCanBeRegister canBeRegister, @NotNull IRLocalVarFactory tempVarFactory, @NotNull LSCallingConventionProvider callingConventionProvider, @NotNull IRConverterLayer nextLayer) {
		super(nextLayer);
	...
	public void process(@NotNull IRInstruction instruction) {
		...
		case IRRetValue retValue -> {
			...
			if (var != null) {
				final IRVar regArg = var.asRegister(0);
				forward(new IRMove(regArg, var, retValue.location()));
			}
			else {
				final IRVar regArg = new IRVar("", 0, VariableScope.register, value.type());
				forward(new IRMove(regArg, value, retValue.location()));
			}
		}
