 ...
 */
final class LSPreprocessorReplaceVarLayer extends IRConverterAbstractLayer {

	@NotNull private final Map<IRVar, IRVar> registerArgToLocalVar;

	public LSPreprocessorReplaceVarLayer(@NotNull Map<IRVar, IRVar> registerArgToLocalVar, @NotNull IRConverterLayer nextLayer) {
		super(nextLayer);
	...
	public void process(@NotNull IRInstruction instruction) {
		...
		case IRBinary i -> {
			IRValue right = i.right();
			final IRVar rightVar = right.var();
			if (rightVar != null) {
				right = new IRValue(replace(rightVar));
			}
			forward(new IRBinary(replace(i.target()), i.op(), replace(i.left()), right));
		}
		case IRBranch i -> {
			IRValue right = i.right();
			final IRVar rightVar = right.var();
			...
			forward(new IRBranch(i.op(), replace(i.left()), right, i.target(), i.nextLabel()));
		}
		case IRCall i -> {
		...
		case IRCompare i -> {
			...
			forward(new IRCompare(replace(i.target()), i.op(), replace(i.left()), right));
		}
		...
		case IRRetValue i -> {
			...
			if (var != null) {
				forward(new IRRetValue(replace(var)));
			}
			else {
				forward(i);
			}
