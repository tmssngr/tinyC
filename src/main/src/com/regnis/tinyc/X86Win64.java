...
public final class X86Win64 extends AsmWriter {
	...
	protected void writeBinary(IRBinary binary) throws IOException {
		...
		case Mul -> {
			if (rightVar != null) {
				...
				final int rightReg = getRegisterVarRegisterIndex(rightVar);
				...
				if (getTypeSize(rightVar.type()) != 8) {
				...
				if (targetReg == leftReg) {
					...
					// maybe combine with movsx above
					writeIndented("mov rax, " + leftRegName);
					writeIndented("imul rax, " + right.value());
					writeIndented("mov " + targetRegName + ", rax");
				}
			}
		}
		case Div, Mod -> {
			if (rightVar != null) {
				...
				Utils.assertTrue(Objects.equals(type, rightVar.type()));
				...
				if (size != 8) {
					...
					writeMovx(rightRegName, rightReg, rightVar, signed);
				}
				...
				writeIndented("mov cx, " + right.value());
				writeIndented("cqo"); // rdx := signbit(rax)
				writeIndented("idiv cx");
			}
		}
	...
	protected void writeBranch(IRBranch branch) throws IOException {
		...
		final int leftReg = getRegisterVarRegisterIndex(branch.left());
	...
	protected void writeCast(IRCast cast) throws IOException {
		...
		if (targetSize > sourceSize) {
			if (source.type() == Type.U8) {
				writeIndented("movzx " + getRegName(targetReg, targetSize) + ", " + getRegName(sourceReg, sourceSize));
			}
			else if (sourceSize == 4) {
				writeIndented("movsxd " + getRegName(targetReg, targetSize) + ", " + getRegName(sourceReg, sourceSize));
			}
			else {
				writeIndented("movsx " + getRegName(targetReg, targetSize) + ", " + getRegName(sourceReg, sourceSize));
			}
		}
	...
	protected void writeMove(IRMove move) throws IOException {
		...
		if (sourceVar != null) {
			...
			if (sourceVar.scope() == VariableScope.register) {
				if (target.scope() == VariableScope.register) {
					writeIndented("mov " + getRegName(target) + ", " + getRegName(sourceVar));
					return;
				}

				addrOf(addrReg, target);
				writeIndented("mov [" + getRegName(addrReg) + "], " + getRegName(sourceVar));
				return;
			}

			Utils.assertTrue(target.scope() == VariableScope.register);
			addrOf(addrReg, sourceVar);
	...
	protected void writeRetValue(IRRetValue retValue) throws IOException {
		...
		if (var != null) {
			final int valueReg = getRegisterVarRegisterIndex(var);
			writeIndented("mov rax, " + getRegName(valueReg));
		}
		else {
			writeIndented("mov rax, " + value);
		}
	}
	...
	private void writeBinary(String op, IRBinary binary) throws IOException {
		...
		if (rightVar != null) {
			...
			final String targetRegName = getRegName(targetReg, left);
			if (targetReg == leftReg) {
				writeIndented(op + " " + targetRegName + ", " + right.value());
			}
			else {
				writeIndented("mov " + targetRegName + ", " + leftRegName);
				writeIndented(op + " " + targetRegName + ", " + right.value());
