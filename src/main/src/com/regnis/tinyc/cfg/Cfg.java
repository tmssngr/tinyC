...
public final class Cfg {
	...
	public BasicBlock get(@NotNull String name) {
		...
		if (block == null) {
			throw new IllegalStateException("Missing block '" + name + "'");
		}
		return block;
	}
	...
	public void setPredecessors() {
		...
		visitPreOrder(null, (name, successor) -> {
			...
			if (predecessors.contains(name)) {
				throw new IllegalStateException("block '" + successor + "': duplicate predecessor '" + name + "'");
			}
	...
	public void eliminateCriticalEdges() {
		...
		visitPreOrder(block -> {
			...
			for (String successor : successors) {
				final BasicBlock successorBlock = get(successor);
				if (successorBlock.predecessors().size() > 1) {
