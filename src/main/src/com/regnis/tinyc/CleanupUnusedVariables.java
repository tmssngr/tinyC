...
public abstract class CleanupUnusedVariables {
	...
	protected void process(IRInstruction instruction) {
		...
		case IRArrayAccess access -> readWrite(access.addr(), List.of(access.array(), access.index()));
		case IRAddrOf addrOf -> readWrite(addrOf.target(), List.of(addrOf.source()));
		case IRAddrOfArray addrOf -> readWrite(addrOf.addr(), List.of(addrOf.array(), addrOf.index()));
