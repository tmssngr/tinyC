...
abstract class AsmWriter {
	...
	protected abstract void writeBinary(IRBinary.Op op, int targetReg, int leftReg, Type leftType, int value) throws IOException;
	...
	protected abstract void writeCompare(IRCompare.Op op, int targetReg, int leftReg, Type leftRightType, int rightValue) throws IOException;
	...
	protected void writeInstruction(IRInstruction instruction) throws IOException {
		...
		case IRBinary binary -> {
			...
			if (rightVar != null) {
				...
				writeBinary(binary.op(), target.index(), left.index(), left.type(), rightVar.index(), rightVar.type());
			}
			else {
				writeBinary(binary.op(), target.index(), left.index(), left.type(), right.value());
			}
		}
		...
		case IRCompare compare -> {
			...
			if (rightVar != null) {
				...
				Utils.assertTrue(left.scope() == VariableScope.register);
				Utils.assertTrue(rightVar.scope() == VariableScope.register);
				writeCompare(compare.op(), target.index(), left.index(), rightVar.index(), left.type());
			}
			else {
				Utils.assertTrue(target.scope() == VariableScope.register);
				Utils.assertTrue(left.scope() == VariableScope.register);
				writeCompare(compare.op(), target.index(), left.index(), left.type(), right.value());
			}
		}
