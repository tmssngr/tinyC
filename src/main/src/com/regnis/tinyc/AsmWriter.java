...
abstract class AsmWriter {
	...
	protected abstract void writeBinaryRegReg(IRBinary.Op op, int targetReg, int leftReg, Type leftType, int rightReg, Type rightType) throws IOException;

	protected abstract void writeBinaryRegValue(IRBinary.Op op, int targetReg, int leftReg, Type leftType, int value) throws IOException;

	protected abstract void writeBranchRegReg(IRCompare.Op op, int leftReg, int rightReg, Type leftType, String targetLabel) throws IOException;

	protected abstract void writeBranchRegValue(IRCompare.Op op, int leftReg, int rightValue, Type leftType, String targetLabel) throws IOException;
	...
	protected abstract void writeCompareRegReg(IRCompare.Op op, int targetReg, int leftReg, int rightReg, Type leftRightType) throws IOException;

	protected abstract void writeCompareRegValue(IRCompare.Op op, int targetReg, int leftReg, Type leftRightType, int rightValue) throws IOException;
	...
	protected void writeInstruction(IRInstruction instruction) throws IOException {
		...
		case IRBinary binary -> {
			...
			if (rightVar != null) {
				Utils.assertTrue(rightVar.scope() == VariableScope.register);
				writeBinaryRegReg(binary.op(), target.index(), left.index(), left.type(), rightVar.index(), rightVar.type());
			}
			else {
				writeBinaryRegValue(binary.op(), target.index(), left.index(), left.type(), right.value());
			}
		}
		case IRBranch branch -> {
			final IRCompare.Op op = branch.op();
			final IRVar leftVar = branch.left();
			Utils.assertTrue(leftVar.scope() == VariableScope.register);
			final IRValue right = branch.right();
			final IRVar rightVar = right.var();
			if (rightVar != null) {
				Utils.assertTrue(rightVar.scope() == VariableScope.register);
				writeBranchRegReg(op, leftVar.index(), rightVar.index(), leftVar.type(), branch.target());
			}
			else {
				writeBranchRegValue(op, leftVar.index(), right.value(), leftVar.type(), branch.target());
			}
		}
		case IRCall call -> {
		...
		case IRCompare compare -> {
			...
			if (rightVar != null) {
				...
				Utils.assertTrue(left.scope() == VariableScope.register);
				Utils.assertTrue(rightVar.scope() == VariableScope.register);
				writeCompareRegReg(compare.op(), target.index(), left.index(), rightVar.index(), left.type());
			}
			else {
				Utils.assertTrue(target.scope() == VariableScope.register);
				Utils.assertTrue(left.scope() == VariableScope.register);
				writeCompareRegValue(compare.op(), target.index(), left.index(), left.type(), right.value());
			}
		}
	...
	protected List<List<IRVar>> getCallArgs(List<IRInstruction> instructions) {
