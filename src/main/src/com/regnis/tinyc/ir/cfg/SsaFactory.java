...
public final class SsaFactory {
	...
	private ProcessingBlock createBlock(String name, Map<IRVar, IRVar> prevMapping) {
		...
		for (IRVar var : liveBefore) {
			...
			if (replacement == null) {
				Utils.assertTrue(var.scope() == VariableScope.global || !varFactory.canBeRegister(var));
				continue;
			}
	...
	private IRInstruction convert(IRInstruction instruction, Map<IRVar, IRVar> liveVars) {
		return switch (instruction) {
			case IRAddrOf i -> {
				final IRVar source = i.source();
				Utils.assertTrue(!varFactory.canBeRegister(source));
	...
	private IRVar source(IRVar var, Map<IRVar, IRVar> live) {
		if (var.scope() == VariableScope.global || !varFactory.canBeRegister(var)) {
			return var;
		}
