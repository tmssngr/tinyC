...
public abstract class IRVarReplacer {
	...
	public IRInstruction replaceFor(IRInstruction instruction) {
		return switch (instruction) {
			...
			case IRArrayAccess access -> new IRArrayAccess(replace(access.addr()), replace(access.array()), replace(access.index()), access.location());
			case IRAddrOf addrOf -> new IRAddrOf(replace(addrOf.target()), replace(addrOf.source()), addrOf.location());
			case IRAddrOfArray addrOf -> new IRAddrOfArray(replace(addrOf.addr()), replace(addrOf.array()), replace(addrOf.index()), addrOf.varIsArray(), addrOf.location());
