...
//		writeFunctionEpilog(rspOffset, nonvolatileRegistersToPushPop, callArgSpace);
	...
	protected void writeBranchRegReg(IRCompare.Op op, int leftReg, int rightReg, Type leftRightType, String targetLabel) throws IOException {
		...
		switch (op) {
		case Lt -> writeBranchRegReg(signed ? "lt" : "ult", "ult",
		                             leftReg, rightReg, byteCount, targetLabel);
		case LtEq -> writeBranchRegReg(signed ? "le" : "ule", "ule",
		                               leftReg, rightReg, byteCount, targetLabel);
		case GtEq -> writeBranchRegReg(signed ? "ge" : "uge", "uge",
		                               leftReg, rightReg, byteCount, targetLabel);
		case Gt -> writeBranchRegReg(signed ? "gt" : "ugt", "ugt",
		                             leftReg, rightReg, byteCount, targetLabel);
		case Equals -> {
			final int localJumpIndex = ++this.localJumpIndex;
	...
	protected void writeBranchRegValue(IRCompare.Op op, int leftReg, int rightValue, Type leftType, String targetLabel) throws IOException {
		...
		final boolean signed = leftType != Type.U8;
		switch (op) {
		case Lt -> writeBranchRegLit(signed ? "lt" : "ult", "ult",
		                             leftReg, rightValue, byteCount, targetLabel);
		case LtEq -> writeBranchRegLit(signed ? "le" : "ule", "ule",
		                               leftReg, rightValue, byteCount, targetLabel);
		case GtEq -> writeBranchRegLit(signed ? "ge" : "uge", "uge",
		                               leftReg, rightValue, byteCount, targetLabel);
		case Gt -> writeBranchRegLit(signed ? "gt" : "ugt", "ugt",
		                             leftReg, rightValue, byteCount, targetLabel);
		case Equals -> {
			final int localJumpIndex = ++this.localJumpIndex;
			final String nextLabel = "." + localJumpIndex;
	...
	protected void writeCompareRegReg(IRCompare.Op op, int targetReg, int leftReg, int rightReg, Type leftRightType) throws IOException {
		...
		final boolean signed = leftRightType != Type.U8;
		switch (op) {
		case Lt -> writeCompareRegReg(signed ? "lt" : "ult", "ult",
		                              signed ? "ge" : "uge", "uge", targetReg, leftReg, rightReg, byteCount);
		case LtEq -> writeCompareRegReg(signed ? "le" : "ule", "ule",
		                                signed ? "gt" : "ugt", "ugt", targetReg, leftReg, rightReg, byteCount);
		case GtEq -> writeCompareRegReg(signed ? "ge" : "uge", "uge",
		                                signed ? "lt" : "ult", "ult", targetReg, leftReg, rightReg, byteCount);
		case Gt -> writeCompareRegReg(signed ? "gt" : "ugt", "ugt",
	...
	protected void writeCompareRegValue(IRCompare.Op op, int targetReg, int leftReg, Type leftRightType, int rightValue) throws IOException {
		...
		final boolean signed = leftRightType != Type.U8;
		switch (op) {
		case Lt -> writeCompareRegLit(signed ? "lt" : "ult", "ult",
		                              signed ? "ge" : "uge", "uge", targetReg, leftReg, rightValue, byteCount);
		case LtEq -> writeCompareRegLit(signed ? "le" : "ule", "ule",
		                                signed ? "gt" : "ugt", "ugt", targetReg, leftReg, rightValue, byteCount);
		case GtEq -> writeCompareRegLit(signed ? "ge" : "uge", "uge",
		                                signed ? "lt" : "ult", "ult", targetReg, leftReg, rightValue, byteCount);
		case Gt -> writeCompareRegLit(signed ? "gt" : "ugt", "ugt",
		                              signed ? "le" : "ule", "ule", targetReg, leftReg, rightValue, byteCount);
		case Equals -> {
			final int localJumpIndex = ++this.localJumpIndex;
			final String falseLabel = ".ne" + localJumpIndex;
			final String nextLabel = "." + localJumpIndex;
	...
	private void writeBranchRegReg(String msbTrueOp, String lsbTrueOp,
	                               int leftReg, int rightReg, int byteCount, String targetLabel) throws IOException {
		final int localJumpIndex = ++this.localJumpIndex;
		final String nextLabel = "." + localJumpIndex;
		for (int i = 0; i < byteCount; i++, leftReg++, rightReg++) {
			writeIndented("cp   " + regName(leftReg) + ", " + regName(rightReg));
			final boolean isLeastSignificantByte = i == byteCount - 1;
			if (isLeastSignificantByte) {
				writeIndented("jr   " + (i == 0 ? msbTrueOp : lsbTrueOp) + ", " + targetLabel);
			}
			else {
				writeIndented("jr   " + (i == 0 ? msbTrueOp : lsbTrueOp) + ", " + nextLabel);
				writeIndented("jr   ne, " + targetLabel);
			}
		}
	...
	private void writeBranchRegLit(String msbTrueOp, String lsbTrueOp,
	                               int leftReg, int rightValue, int byteCount, String targetLabel) throws IOException {
		final int localJumpIndex = ++this.localJumpIndex;
		final String nextLabel = "." + localJumpIndex;
		final int[] bytes = toBytes(rightValue, byteCount);
		for (int i = 0; i < byteCount; i++, leftReg++) {
	...
	private void writeCompareRegReg(String msbTrueOp, String lsbTrueOp,
	...
	private void writeCompareRegLit(String msbTrueOp, String lsbTrueOp,
	                                String msbFalseOp, String lsbFalseOp, int targetReg, int leftReg, int rightValue, int byteCount) throws IOException {
		final int localJumpIndex = ++this.localJumpIndex;
		final String trueLabel = ".true" + localJumpIndex;
		final String falseLabel = ".false" + localJumpIndex;
		final String nextLabel = "." + localJumpIndex;
