...
final class Z8AsmWriter extends AsmWriter {
	...
	protected void writeFunction(IRFunction function) throws IOException {
		...
		final int localVarSpace = stackOffsets.localVarSpace();
...
//		writeFunctionProlog(rspOffset, nonvolatileRegistersToPushPop, callArgSpace);
		writeFunctionProlog(localVarSpace, firstGlobberedNonVolatileRegister, globberedNonVolatileRegisterCount);

		writeInstructions(instructions);

//		writeFunctionEpilog(rspOffset, nonvolatileRegistersToPushPop, callArgSpace);
		writeFunctionEpilog(localVarSpace, firstGlobberedNonVolatileRegister, globberedNonVolatileRegisterCount);
		stackOffsets = null;
	}
	...
	protected void writeAddrOf(int addrReg, IRVar source) throws IOException {
		...
		if (scope == VariableScope.global) {
			final String globalVarLabel = getGlobalVarName(source.index());
		...
		if (offset > 0) {
			writeIndented("add  " + regName(addrReg + 1) + ", #%" + byteToHex(offset));
			offset >>= 8;
			writeIndented("adc  " + regName(addrReg) + ", #%" + byteToHex(offset));
		}
	}
	...
	protected void writeAddrOfArray(int addrReg, IRVar arrayOrPointer) throws IOException {
		if (arrayOrPointer.scope() == VariableScope.global) {
			final String globalVarLabel = getGlobalVarName(arrayOrPointer.index());
			writeIndented("ld   " + regName(addrReg) + ", #hi(" + globalVarLabel + ")");
			writeIndented("ld   " + regName(addrReg + 1) + ", #lo(" + globalVarLabel + ")");
			return;
		}
	...
	@Override
	protected void writeBinaryRegReg(IRBinary.Op op, int targetReg, int leftReg, Type leftType, int rightReg, Type rightType) throws IOException {
	...
	@Override
	protected void writeBinaryRegValue(IRBinary.Op op, int targetReg, int leftReg, Type leftType, int value) throws IOException {
		Utils.assertTrue(targetReg == leftReg);
		switch (op) {
		case Add -> {
			final int byteCount = getByteCount(leftType);
			if (byteCount == 1) {
				if (value == 1) {
					writeIndented("inc  " + regName(targetReg));
					return;
				}
				if (value == -1) {
					writeIndented("dec  " + regName(targetReg));
					return;
				}
			}
			else if (byteCount == 2 && (leftReg & 1) == 0) {
				if (value == 1) {
					writeIndented("incw " + regName(targetReg));
					return;
				}
				if (value == -1) {
					writeIndented("decw " + regName(targetReg));
					return;
				}
			}
			targetReg += byteCount - 1;
		...
		case And -> {
			new LogicLiteral() {
				@Override
				protected String getCommand(String regName, int byteValue) {
					if (byteValue == 0xFF) {
						return null;
					}
					if (byteValue == 0) {
					...
					return "and  " + regName + ", #%" + byteToHex(byteValue);
				}
			}.write(targetReg, leftReg, leftType, value);
		}
		case Or -> {
			new LogicLiteral() {
				@Override
				protected String getCommand(String regName, int byteValue) {
					...
					if (byteValue == 0xFF) {
						return "ld  " + regName + ", #%" + byteToHex(byteValue);
					}
					return "or  " + regName + ", #%" + byteToHex(byteValue);
				}
			}.write(targetReg, leftReg, leftType, value);
		}
		case Xor -> {
			new LogicLiteral() {
				...
				protected String getCommand(String regName, int byteValue) {
					...
					return "xor " + regName + ", #%" + byteToHex(byteValue);
				}
			}.write(targetReg, leftReg, leftType, value);
		}
		case ShiftLeft -> {
			...
			if (value == 8) {
				for (int i = 0; i < byteCount; i++, targetReg++) {
					writeIndented("ld   " + regName(targetReg) + ", " + regName(targetReg + 1));
				}
				writeIndented("clr  " + regName(targetReg));
				return;
			}
			if (value == 4 && byteCount == 1) {
				writeIndented("and  " + regName(targetReg) + ", #%0F");
				writeIndented("swap " + regName(targetReg));
				return;
			}

			for (; value > 0; value--) {
				writeIndented("rcf");
				for (int reg = targetReg + byteCount; reg-- > targetReg; ) {
					writeIndented("rlc  " + regName(reg));
				}
			}
		}
		case ShiftRight -> {
			...
			if (value == 8) {
				for (int reg = targetReg + byteCount; reg-- > targetReg; ) {
					writeIndented("ld   " + regName(reg) + ", " + regName(reg - 1));
				}
		...
		case Mul -> mulDivMod("call %00BA ; mul", leftReg, leftType, value);
		case Div -> mulDivMod("call %00E0 ; div", leftReg, leftType, value);
		case Mod -> mulDivMod("call %011F ; mod", leftReg, leftType, value);
		}
	}

	@Override
	protected void writeBranchRegReg(IRCompare.Op op, int leftReg, int rightReg, Type leftRightType, String targetLabel) throws IOException {
		...
		if (byteCount == 1) {
			...
			final String condition = compareOpToJumpCondition(op, signed);
			writeIndented("jr   " + condition + ", " + targetLabel);
			return;
		}

		switch (op) {
		case Lt -> writeBranchRegReg(signed ? "lt" : "ult", "ult", "ult",
		                             leftReg, rightReg, byteCount, targetLabel);
		case LtEq -> writeBranchRegReg(signed ? "lt" : "ult", "ult", "ule",
		                               leftReg, rightReg, byteCount, targetLabel);
		case GtEq -> writeBranchRegReg(signed ? "gt" : "ugt", "ugt", "uge",
		                               leftReg, rightReg, byteCount, targetLabel);
		case Gt -> writeBranchRegReg(signed ? "gt" : "ugt", "ugt", "ugt",
		                             leftReg, rightReg, byteCount, targetLabel);
		case Equals -> {
			final int localJumpIndex = ++this.localJumpIndex;
			final String nextLabel = ".notEquals" + localJumpIndex;
			// we compare from lower to higher byte because the lower one more often changes;
			// that way we potentially can avoid comparing the upper byte if it is equal
			leftReg += byteCount - 1;
			rightReg += byteCount - 1;
			for (int i = byteCount; i-- > 0; leftReg--, rightReg--) {
				writeIndented("cp   " + regName(leftReg) + ", " + regName(rightReg));
				final boolean isLast = i == 0;
				if (isLast) {
					writeIndented("jr   eq, " + targetLabel);
				}
				else {
					writeIndented("jr   ne, " + nextLabel);
				}
			}
		...
		case NotEquals -> {
			...
			for (int i = byteCount; i-- > 0; leftReg--, rightReg--) {
				writeIndented("cp   " + regName(leftReg) + ", " + regName(rightReg));
				writeIndented("jr   ne, " + targetLabel);
			}
		}
		}
	}
	...
	@Override
	protected void writeBranchRegValue(IRCompare.Op op, int leftReg, int rightValue, Type leftType, String targetLabel) throws IOException {
		targetLabel = escapeLabel(targetLabel);
		...
		if (byteCount == 1) {
			writeIndented("cp   " + regName(leftReg) + ", #%" + byteToHex(rightValue));
		...
		final int[] bytes = toBytes(rightValue, byteCount);

		switch (op) {
		case Lt -> writeBranchRegLit(signed ? "lt" : "ult", "ult", "ult",
		                             leftReg, byteCount, targetLabel, bytes);
		case LtEq -> writeBranchRegLit(signed ? "lt" : "ult", "ult", "ule",
		                               leftReg, byteCount, targetLabel, bytes);
		case GtEq -> writeBranchRegLit(signed ? "gt" : "ugt", "ugt", "uge",
		                               leftReg, byteCount, targetLabel, bytes);
		case Gt -> writeBranchRegLit(signed ? "gt" : "ugt", "ugt", "ugt",
		                             leftReg, byteCount, targetLabel, bytes);
		case Equals -> {
			final int localJumpIndex = ++this.localJumpIndex;
			final String nextLabel = ".notEquals" + localJumpIndex;
			...
			// that way we potentially can avoid comparing the upper byte if it is equal
			leftReg += byteCount - 1;
			for (int i = byteCount; i-- > 0; leftReg--) {
				writeIndented("cp   " + regName(leftReg) + ", #%" + byteToHex(bytes[i]));
				final boolean isLast = i == 0;
				if (isLast) {
					writeIndented("jr   eq, " + targetLabel);
				}
	...
	@Override
	protected void writeCompareRegReg(IRCompare.Op op, int targetReg, int leftReg, int rightReg, Type leftRightType) throws IOException {
		final boolean signed = leftRightType != Type.U8;
		final int byteCount = getByteCount(leftRightType);
		...
		if (byteCount == 1) {
			...
			final String condition = compareOpToJumpCondition(op, signed);
			writeIndented("jr   " + condition + ", " + trueLabel);
		...
		switch (op) {
		case Lt -> writeCompareRegReg(signed ? "lt" : "ult", "ult", "ult",
		                              targetReg, leftReg, rightReg, byteCount);
		case LtEq -> writeCompareRegReg(signed ? "lt" : "ult", "ult", "ule",
		                                targetReg, leftReg, rightReg, byteCount);
		case GtEq -> writeCompareRegReg(signed ? "gt" : "ugt", "ugt", "uge",
		                                targetReg, leftReg, rightReg, byteCount);
		case Gt -> writeCompareRegReg(signed ? "gt" : "ugt", "ugt", "ugt",
		                              targetReg, leftReg, rightReg, byteCount);
		case Equals -> {
			final int localJumpIndex = ++this.localJumpIndex;
	...
	@Override
	protected void writeCompareRegValue(IRCompare.Op op, int targetReg, int leftReg, Type leftRightType, int rightValue) throws IOException {
		final boolean signed = leftRightType != Type.U8;
		final int byteCount = getByteCount(leftRightType);
		...
		if (byteCount == 1) {
			...
			final String condition = compareOpToJumpCondition(op, signed);
			final int localJumpIndex = ++this.localJumpIndex;
			final String trueLabel = "." + condition + localJumpIndex;
			final String nextLabel = "." + localJumpIndex;
			writeIndented("jr   " + condition + ", " + trueLabel);
			writeIndented("ld   " + regName(targetReg) + ", #0  ; false");
			writeIndented("jr   " + nextLabel);
		...
		final int[] bytes = toBytes(rightValue, byteCount);
		switch (op) {
		case Lt -> writeCompareRegLit(signed ? "lt" : "ult", "ult", "ult",
		                              targetReg, leftReg, byteCount, bytes);
		case LtEq -> writeCompareRegLit(signed ? "lt" : "ult", "ult", "ule",
		                                targetReg, leftReg, byteCount, bytes);
		case GtEq -> writeCompareRegLit(signed ? "gt" : "ugt", "ugt", "uge",
		                                targetReg, leftReg, byteCount, bytes);
		case Gt -> writeCompareRegLit(signed ? "gt" : "ugt", "ugt", "ugt",
		                              targetReg, leftReg, byteCount, bytes);
		case Equals -> {
			final int localJumpIndex = ++this.localJumpIndex;
			final String falseLabel = ".ne" + localJumpIndex;
			final String nextLabel = "." + localJumpIndex;
			for (int i = 0; i < byteCount; i++, leftReg++) {
				writeIndented("cp   " + regName(leftReg) + ", #%" + byteToHex(bytes[i]));
				writeIndented("jr   ne, " + falseLabel);
			}
	...
	protected void writeLiteral(int targetReg, Type type, int value) throws IOException {
		...
		final int[] bytes = toBytes(value, byteCount);
		for (int i = 0; i < byteCount; i++) {
			writeIndented("ld   " + regName(targetReg + i) + ", #%" + byteToHex(bytes[i]));
		}
	}
	...
	protected void writeString(int targetReg, Type targetType, int stringLiteralIndex) throws IOException {
		final String stringLabel = getStringLiteralName(stringLiteralIndex);
	...
	protected void writeUnary(IRUnary.Op op, int targetReg, Type targetType, int sourceReg, Type sourceType) throws IOException {
		...
		case Neg -> {
			...
			writeBinaryRegReg(IRBinary.Op.Sub, targetReg, targetReg, targetType, sourceReg, sourceType);
		}
		case Not -> {
			if (sourceReg != targetReg) {
	...
	private void mulDivMod(String call, int leftReg, Type leftType, int rightValue) throws IOException {
		if (leftType == Type.I16) {
			writeIndented("ld   %12, " + regName(leftReg));
			writeIndented("ld   %13, " + regName(leftReg + 1));
			final int[] bytes = toBytes(rightValue, 2);
			writeIndented("ld   %14, #%" + byteToHex(bytes[0]));
			writeIndented("ld   %15, #%" + byteToHex(bytes[1]));
			writeIndented("srp  #%10");
			writeIndented(call);
			writeIndented("srp  #%20");
			writeIndented("ld   " + regName(leftReg) + ", %12");
	...
	@NotNull
	@Override
	protected String escapeLabel(String label) throws IOException {
		final StringBuilder buffer = new StringBuilder();
	...
	private void writeBranchRegReg(String msbCondition, String middleCondition, String lsbCondition,
	                               int leftReg, int rightReg, int byteCount, String targetLabel) throws IOException {
		Utils.assertTrue(byteCount > 1);
		final int localJumpIndex = ++this.localJumpIndex;
		final String nextLabel = "." + msbCondition + localJumpIndex;

		for (int i = 0; i < byteCount; i++, leftReg++, rightReg++) {
			writeIndented("cp   " + regName(leftReg) + ", " + regName(rightReg));
			final boolean isMsb = i == 0;
			final boolean isLsb = i == byteCount - 1;
			if (isMsb) {
				writeIndented("jr   " + msbCondition + ", " + targetLabel);
				writeIndented("jr   ne, " + nextLabel);
			}
			else if (!isLsb) {
				writeIndented("jr   " + middleCondition + ", " + targetLabel);
				writeIndented("jr   ne, " + nextLabel);
			}
			else {
				writeIndented("jr   " + lsbCondition + ", " + targetLabel);
			}
		}
	...
	private void writeBranchRegLit(String msbCondition, String middleCondition, String lsbCondition,
	                               int leftReg, int byteCount, String targetLabel, int[] bytes) throws IOException {
		Utils.assertTrue(byteCount > 1);
		final int localJumpIndex = ++this.localJumpIndex;
		final String nextLabel = "." + msbCondition + localJumpIndex;
	...
	private void writeCompareRegReg(String msbCondition, String middleCondition, String lsbCondition,
	                                int targetReg, int leftReg, int rightReg, int byteCount) throws IOException {
		final int localJumpIndex = ++this.localJumpIndex;
		final String trueLabel = ".true" + localJumpIndex;
		...
		for (int i = 0; i < byteCount; i++, leftReg++, rightReg++) {
			...
			final boolean isLsb = i == byteCount - 1;
			if (isMsb) {
				writeIndented("jr   " + msbCondition + ", " + trueLabel);
				writeIndented("jr   ne, " + falseLabel);
			}
			else if (!isLsb) {
				writeIndented("jr   " + middleCondition + ", " + trueLabel);
				writeIndented("jr   ne, " + falseLabel);
			}
			else {
				writeIndented("jr   " + lsbCondition + ", " + trueLabel);
			}
		}
	...
	private void writeCompareRegLit(String msbCondition, String middleCondition, String lsbCondition,
	                                int targetReg, int leftReg, int byteCount, int[] bytes) throws IOException {
		final int localJumpIndex = ++this.localJumpIndex;
		final String trueLabel = ".true" + localJumpIndex;
		final String falseLabel = ".false" + localJumpIndex;
		final String nextLabel = "." + localJumpIndex;
	...
	private void writeFunctionProlog(int localVarSpace, int firstNonVolatileRegister, int count) throws IOException {
		if (localVarSpace > 0) {
			writeIndented("ld   %30, SPH");
			writeIndented("ld   %31, SPL");
			writeIndented("sub  %31, #%" + byteToHex(localVarSpace));
			localVarSpace >>= 8;
			writeIndented("sbc  %30, #%" + byteToHex(localVarSpace));
			// order matters! if an overflow in SPH happened, better let an
			// interrupt overwrite something further below in the stack than
			// in the active stack area
			writeIndented("ld   SPH, %30");
			writeIndented("ld   SPL, %31");
		}
		if (count > 0) {
...
*/
	...
	private void writeFunctionEpilog(int localVarSpace, int firstNonvolatileRegistersToPop, int count) throws IOException {
/*
		if (callArgSpace > 0) {
...
*/
		...
		if (localVarSpace > 0) {
			writeIndented("ld   %30, SPH");
			writeIndented("ld   %31, SPL");
			writeIndented("add  %31, #%" + byteToHex(localVarSpace));
			localVarSpace >>= 8;
			writeIndented("adc  %30, #%" + byteToHex(localVarSpace));
			// if SPH is unchanged, the order does not matter
			// but if it changed, it will be higher, so the worst that
			// can happen by an interrupt is overwriting stack memory
	...
	@NotNull
	private static String compareOpToJumpCondition(IRCompare.Op op, boolean signed) {
		return switch (op) {
			case Lt -> signed ? "lt" : "ult";
			case LtEq -> signed ? "le" : "ule";
			case GtEq -> signed ? "ge" : "uge";
			case Gt -> signed ? "gt" : "ugt";
			case Equals -> "eq";
			case NotEquals -> "ne";
		};
	}
	...
	private abstract class LogicLiteral {
		@Nullable
		protected abstract String getCommand(String regName, int byteValue);

		public void write(int targetReg, int leftReg, Type leftType, int rightValue) throws IOException {
			Utils.assertTrue(targetReg == leftReg);
			final int byteCount = getByteCount(leftType);
			for (int reg = targetReg + byteCount; reg-- > targetReg; ) {
				final int byteValue = rightValue & 0xFF;
				final String regName = regName(reg);
				final String command = getCommand(regName, byteValue);
				if (command != null) {
