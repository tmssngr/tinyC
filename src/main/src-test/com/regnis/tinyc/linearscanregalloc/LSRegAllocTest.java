...
public class LSRegAllocTest {
	...
	public void testSimplest() {
		...
		final IRFunction regAllocFunction = LSRegAlloc.process(function, null, 3, callingConventionProvider, Type.I64);
	...
	public void testSplitLiveInterval() {
		...
		assertEquals(new IRCall(varB.asRegister(rRet), Type.U8, "foo", List.of(varA.asRegister(rArg1))), it.next());
		assertEquals(new IRAddrOf(varMVA.asRegister(rSpill), varA), it.next());
	...
	public void testPrintString() {
		...
		final LSCallingConventionProvider callingConventionProvider = (targetType, argTypes) -> LSCallingConvention.createX86CallingConvention(2, 0);
		final IRFunction regAllocFunction = LSRegAlloc.process(function, null, 5, callingConventionProvider, Type.I64);
		final Iterator<IRInstruction> it = regAllocFunction.instructions().iterator();
		...
		assertEquals(new IRCall(varLength.asRegister(rRet), Type.U8, "strlen", List.of(varStr.asRegister(rArg1))), it.next());
		assertEquals(new IRMove(varStr.asRegister(rArg1), varStr.asRegister(rNV1)), it.next());
		assertEquals(new IRMove(varLength.asRegister(rArg2), varLength.asRegister(rRet)), it.next());
		assertEquals(new IRCall(null, Type.VOID, "printStringLength", List.of(varStr.asRegister(rArg1), varLength.asRegister(rArg2))), it.next());
		assertFalse(it.hasNext());
	}
	...
	public void testStackVar() {
		...
		final Iterator<IRInstruction> it = regAllocFunction.instructions().iterator();
		assertEquals(new IRLiteral(varA1, 1), it.next());
		assertEquals(new IRLiteral(varB0, 2), it.next());

		assertEquals(new IRAddrOf(varSpill2, varB), it.next());
		assertEquals(new IRMemStore(varSpill2, varB0), it.next());

		assertEquals(new IRLiteral(varC0, 3), it.next());
		...
		assertEquals(new IRCall(null, Type.VOID, "print", List.of(varA1)), it.next());
		...
		assertEquals(new IRCall(null, Type.VOID, "print", List.of(varT1)), it.next());

		assertEquals(new IRAddrOf(varSpill2, varC), it.next());
		assertEquals(new IRMemLoad(varC1, varSpill2), it.next());

		assertEquals(new IRCall(null, Type.VOID, "print", List.of(varC1)), it.next());
		assertFalse(it.hasNext());
	}
}