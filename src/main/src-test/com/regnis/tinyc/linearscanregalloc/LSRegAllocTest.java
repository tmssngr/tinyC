...
public class LSRegAllocTest {
	...
	public void testSimplest() {
		...
		final List<IRInstruction> instructions = LSRegAlloc.process(function, false, 3, callingConventionProvider, typeRegisterCountProvider);
	...
	public void testSplitLiveInterval() {
		...
		final IRFunction function = new IRFunction(
				...
				List.of(
						...
						new IRCall(varB, Type.U8, "foo", List.of(varA), Location.DUMMY),
		...
		final LSTypeRegisterCountProvider typeRegisterCountProvider = type -> 1;
		final List<IRInstruction> instructions = LSRegAlloc.process(function, false, 2, callingConventionProvider, typeRegisterCountProvider);
	...
	public void testIf() {
		...
		final IRFunction function = new IRFunction(
				...
				List.of(
						new IRCompare(varTmp, IRCompareOp.Lt, varA, varB, Location.DUMMY),
	...
	public void testPrintString() {
		...
		final IRFunction function = new IRFunction(
				...
				List.of(
						new IRCall(varLength, Type.U8, "strlen", List.of(varStr), Location.DUMMY),
						new IRCall(null, Type.VOID, "printStringLength", List.of(varStr, varLength), Location.DUMMY)
				)
		);
		final LSCallingConventionProvider callingConventionProvider = (targetType, argTypes) -> LSCallingConvention.createX86CallingConvention(2, 0);
		final LSTypeRegisterCountProvider typeRegisterCountProvider = type -> 1;
		final List<IRInstruction> instructions = LSRegAlloc.process(function, false, 5, callingConventionProvider, typeRegisterCountProvider);
		IRTestUtils.assertEqualsInstructions(List.of(
				...
				new IRCall(varLength.asRegister(rRet), Type.U8, "strlen", List.of(varStr.asRegister(rArg1)), Location.DUMMY),
				new IRMove(varStr.asRegister(rArg1), varStr.asRegister(rNV1), Location.DUMMY),
				new IRMove(varLength.asRegister(rArg2), varLength.asRegister(rRet), Location.DUMMY),
				new IRCall(null, Type.VOID, "printStringLength", List.of(varStr.asRegister(rArg1), varLength.asRegister(rArg2)), Location.DUMMY)
		), instructions);
	}
