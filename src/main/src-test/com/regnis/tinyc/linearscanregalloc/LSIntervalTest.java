...
public class LSIntervalTest {
	...
	public void testSplit2() {
		final LSInterval interval = LSInterval.testVar(new IRVar("strlen", 0, VariableScope.parameter, Type.POINTER_U8),
		                                               ...
		                                               List.of(write(0), read(12), write(20), read(34)));
		assertEquals(0, interval.getFrom());
		assertEquals(34, interval.getTo());
		assertEquals(List.of(write(0), read(12), write(20), read(34)), interval.uses());

		final LSInterval child = interval.truncateAndSplit(12);
		assertEquals(0, interval.getFrom());
		assertEquals(12, interval.getTo());
		assertEquals(List.of(write(0), read(12)), interval.uses());

		assertEquals(20, child.getFrom());
		assertEquals(34, child.getTo());
		assertEquals(List.of(write(20), read(34)), child.uses());
	}
	...
	public void testGetSubInterval() {
		final LSInterval i0 = LSInterval.testVar(new IRVar("a", 0, VariableScope.function, Type.I64),
		                                         ...
		                                         List.of(write(0), read(6), read(14)));
	...
	public void testGetTransitionAt() {
		...
		final LSInterval i0 = LSInterval.testVar(var,
		                                         ...
		                                         List.of(write(6), read(16), write(22), read(36)));
		int i = 0;
		for (; i < 38; i++) {
			assertGetTransitionAt(null, i, i0, var);
		}
		...
		assertGetTransitionAt(new Pair<>(var.asRegister(1), var.asRegister(2)), i, i0, var);
		i++;
		for (; i < 38; i++) {
			assertGetTransitionAt(null, i, i0, var);
		}
	...
	private void assertGetTransitionAt(@Nullable Pair<IRVar, IRVar> expectedTransition, int pos, LSInterval i0, IRVar var) {
		assertEquals(expectedTransition, i0.getTransitionAt(pos, var));
	}
}