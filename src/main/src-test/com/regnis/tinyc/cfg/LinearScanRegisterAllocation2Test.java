...
public class LinearScanRegisterAllocation2Test {
	...
	public void testReadmeExample() {
		...
		Assert.assertEquals(List.of(
				                    ...
				                    move("x", nonVolatile1, CALL_ARG_0),

				                    new IRLiteral(reg("y", CALL_ARG_1), 1, Location.DUMMY),
				                    move("y", nonVolatile0, CALL_ARG_1),

				                    new IRCall(reg("a", CALL_RETURN_REG), "foo", List.of(
						                    reg("x", CALL_ARG_0),
						                    reg("y", CALL_ARG_1)
				                    ), Location.DUMMY),
				                    move("x", CALL_ARG_1, nonVolatile1),
				                    move("y", CALL_ARG_0, nonVolatile0),
				                    ...
				                    move("a", CALL_ARG_0, nonVolatile0),
	...
	public void testLongCall() {
		...
		Assert.assertEquals(1, blocks.size());
		Assert.assertEquals(List.of(
				                    new IRLiteral(reg("a", CALL_ARG_0), 10, Location.DUMMY),
				                    new IRLiteral(reg("b", CALL_ARG_1), 11, Location.DUMMY),
