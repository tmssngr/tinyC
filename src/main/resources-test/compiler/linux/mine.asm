...
@isBomb@u8:
        ...
        ; const t.3{r2}, 0
        mov sil, 0
        ; notequals t.1{r0}, t.2{r1}, t.3{r2}
        cmp dil, sil
        setne al
        add rsp, 8
        ret
...
@for_8:
        ; const t.11{r2}, 1
        mov si, 1
        ; lteq t.10{r2}, dc{r1}, t.11{r2}
        cmp di, si
        setle sil
        ; branch t.10{r2}, true, @for_8_body, @for_7_continue
        or sil, sil
        jnz @for_8_body
        ; addrof memVarAddr{r9}, dr
        lea r12, [rsp+38]
...
@printCell@u8@i16@i16:
        ...
        call @isFlag@u8
        ; branch t.9{r0}, false, @no_critical_edge_10, @if_17_then
        or al, al
...
@for_19:
        ; const t.13{r5}, 40
        mov r8w, 40
        ; lt t.12{r5}, column{r0}, t.13{r5}
        cmp ax, r8w
        setl r8b
        ; branch t.12{r5}, true, @for_19_body, @for_19_break
        or r8b, r8b
        jnz @for_19_body
        ; const t.14{r2}, 40
        mov si, 40
        ...
        ; store [memVarAddr{r9}], columnCursor{r4}
        mov [r12], cx
        ; call spacer{r0} = getSpacer@i16@i16@i16@i16[row{r1}, t.14{r2}, rowCursor{r3}, columnCursor{r4}] -> u8
        call @getSpacer@i16@i16@i16@i16
        ; move spacer{r1}, spacer{r0}
        mov dil, al
        ; call printChar@u8[spacer{r1}]
        call @printChar@u8
        ; const t.15{r1}, [string-0]
        lea rdi, [string_0]
        ; call printString@@u8[t.15{r1}]
        call @printString@@u8
        ; addrof memVarAddr{r9}, row
        lea r12, [rsp+36]
        ; load row{r0}, [memVarAddr{r9}]
...
@while_22:
        ...
        ; const t.5{r3}, 0
        mov dx, 0
        ; equals t.4{r3}, value{r1}, t.5{r3}
        cmp di, dx
        sete dl
        ; branch t.4{r3}, false, @while_22, @while_22_break
        or dl, dl
        jz @while_22
        ; 132:9 return count
...
@for_25:
        ; const t.7{r2}, 40
        mov si, 40
        ; lt t.6{r2}, c{r1}, t.7{r2}
        cmp di, si
        setl sil
        ; branch t.6{r2}, true, @for_25_body, @for_24_continue
        or sil, sil
        jnz @for_25_body
        ; addrof memVarAddr{r9}, r
        lea r12, [rsp+32]
...
@for_30_body:
        ...
        ; store [memVarAddr{r9}], row{r0}
        mov [r12], ax
        ; sub t.13{r1}, t.13{r1}, curr_r{r8}
        sub di, bx
        ; call t.12{r0} = abs@i16[t.13{r1}] -> i16
        call @abs@i16
        ; const t.14{r2}, 1
        mov si, 1
        ; gt t.11{r0}, t.12{r0}, t.14{r2}
        cmp ax, si
        setg al
        ; branch t.11{r0}, true, @no_critical_edge_8, @or_2nd_32
        or al, al
        jnz @no_critical_edge_8
        ; addrof memVarAddr{r9}, t.11
        lea r12, [rsp+42]
        ; store [memVarAddr{r9}], t.11{r0}
        mov [r12], al
        jmp @or_2nd_32
@no_critical_edge_8:
        ; addrof memVarAddr{r9}, t.11
...
@or_2nd_32:
        ...
        ; const t.17{r4}, 1
        mov cx, 1
        ; gt t.11{r0}, t.15{r0}, t.17{r4}
        cmp ax, cx
        setg al
@or_next_32:
        ; branch t.11{r0}, false, @for_30_continue, @if_31_then
        or al, al
        jz @for_30_continue
        ; const t.18{r3}, 1
        mov dl, 1
        ...
        ; load column{r2}, [memVarAddr{r9}]
        mov si, [r12]
        ; call setCell@i16@i16@u8[row{r1}, column{r2}, t.18{r3}]
        call @setCell@i16@i16@u8
@for_30_continue:
        ; addrof memVarAddr{r9}, bombs
        lea r12, [rsp+36]
...
@if_36_end:
        ...
        mov al, [r12]
        ; move t.20{r3}, cell{r0}
        mov dl, al
        ; or t.20{r3}, t.20{r3}, 2
        or dl, 2
        ; addrof memVarAddr{r9}, r
        lea r12, [rsp+38]
        ; load r{r1}, [memVarAddr{r9}]
...
@for_35:
        ; const t.13{r1}, 1
        mov di, 1
        ; lteq t.12{r1}, dc{r0}, t.13{r1}
        cmp ax, di
        setle dil
        ; branch t.12{r1}, true, @for_35_body, @for_34_continue
        or dil, dil
        jnz @for_35_body
        ; addrof memVarAddr{r9}, dr
        lea r12, [rsp+36]
...
@for_34:
        ; const t.11{r1}, 1
        mov di, 1
        ; lteq t.10{r1}, dr{r0}, t.11{r1}
        cmp ax, di
        setle dil
        ; branch t.10{r1}, true, @for_34_body, @maybeRevealAround@i16@i16_ret
        or dil, dil
        jnz @for_34_body
...
@if_41_end:
        ...
        ; const t.11{r6}, 27
        mov r9w, 27
        ; equals t.10{r6}, chr{r5}, t.11{r6}
        cmp r8w, r9w
        sete r9b
        ; branch t.10{r6}, true, @main_ret, @if_43_end
        or r9b, r9b
        jnz @main_ret
        ; 234:3 if chr == -8120
        ; const t.13{r6}, -8120
        mov r9w, -8120
        ; equals t.12{r6}, chr{r5}, t.13{r6}
        cmp r8w, r9w
        sete r9b
        ; branch t.12{r6}, true, @if_44_then, @if_44_else
        or r9b, r9b
        jnz @if_44_then
        ; 238:8 if chr == -8112
        ; const t.17{r6}, -8112
        mov r9w, -8112
        ; equals t.16{r6}, chr{r5}, t.17{r6}
        cmp r8w, r9w
        sete r9b
        ; branch t.16{r6}, false, @if_45_else, @if_45_then
        or r9b, r9b
        jz @if_45_else
        jmp @if_45_then
@if_44_then:
        ...
        mov r8w, di
        ; add t.15{r5}, t.15{r5}, 20
        add r8w, 20
        ; sub t.14{r5}, t.14{r5}, 1
        sub r8w, 1
...
@if_45_else:
        ...
        ; const t.20{r6}, -8117
        mov r9w, -8117
        ; equals t.19{r6}, chr{r5}, t.20{r6}
        cmp r8w, r9w
        sete r9b
        ; branch t.19{r6}, false, @if_46_else, @if_46_then
        or r9b, r9b
        jz @if_46_else
        jmp @if_46_then
@if_45_then:
        ...
        ; load curr_r{r1}, [memVarAddr{r9}]
        mov di, [r12]
        ; move t.18{r5}, curr_r{r1}
        mov r8w, di
        ; add t.18{r5}, t.18{r5}, 1
        add r8w, 1
        ; move curr_r{r1}, t.18{r5}
        mov di, r8w
        ; move curr_r{r0}, curr_r{r1}
        mov ax, di
        ; mod curr_r{r3}, curr_r{r0}, 20
...
@if_46_else:
        ...
        ; const t.24{r6}, -8117
        mov r9w, -8117
        ; equals t.23{r6}, chr{r5}, t.24{r6}
        cmp r8w, r9w
        sete r9b
        ; branch t.23{r6}, false, @if_47_else, @if_47_then
        or r9b, r9b
        jz @if_47_else
        jmp @if_47_then
@if_46_then:
        ...
        mov r8w, si
        ; add t.22{r5}, t.22{r5}, 40
        add r8w, 40
        ; sub t.21{r5}, t.21{r5}, 1
        sub r8w, 1
        ; move curr_c{r2}, t.21{r5}
        mov si, r8w
...
@if_47_else:
        ...
        ; const t.28{r6}, -8115
        mov r9w, -8115
        ; equals t.27{r6}, chr{r5}, t.28{r6}
        cmp r8w, r9w
        sete r9b
        ; branch t.27{r6}, false, @if_48_else, @if_48_then
        or r9b, r9b
        jz @if_48_else
        jmp @if_48_then
@if_47_then:
        ...
        ; load curr_c{r2}, [memVarAddr{r9}]
        mov si, [r12]
        ; move t.26{r5}, curr_c{r2}
        mov r8w, si
        ; add t.26{r5}, t.26{r5}, 40
        add r8w, 40
        ; sub t.25{r5}, t.25{r5}, 1
        sub r8w, 1
        ; move curr_c{r2}, t.25{r5}
        mov si, r8w
        ; move curr_c{r0}, curr_c{r2}
        mov ax, si
        ; mod curr_c{r3}, curr_c{r0}, 40
...
@if_48_else:
        ...
        ; const t.31{r6}, 32
        mov r9w, 32
        ; equals t.30{r6}, chr{r5}, t.31{r6}
        cmp r8w, r9w
        sete r9b
        ; branch t.30{r6}, false, @if_49_else, @if_49_then
        or r9b, r9b
        jz @if_49_else
        jmp @if_49_then
@if_48_then:
        ; move t.29{r5}, curr_c{r2}
        mov r8w, si
        ; add t.29{r5}, t.29{r5}, 1
        add r8w, 1
        ; move curr_c{r2}, t.29{r5}
        mov si, r8w
        ; move curr_c{r0}, curr_c{r2}
        mov ax, si
        ; mod curr_c{r3}, curr_c{r0}, 40
...
@if_49_else:
        ...
        ; const t.36{r0}, 13
        mov ax, 13
        ; equals t.35{r0}, chr{r5}, t.36{r0}
        cmp r8w, ax
        sete al
        ; branch t.35{r0}, false, @no_critical_edge_30, @if_52_then
        or al, al
        jz @no_critical_edge_30
        jmp @if_52_then
@if_49_then:
        ; 255:4 if !needsInitialize
        ; notlog t.32{r0}, needsInitialize{r8}
        or bl, bl
        sete al
        ; branch t.32{r0}, false, @no_critical_edge_33, @if_50_then
        or al, al
...
@if_50_then:
        ...
        ; store [memVarAddr{r9}], cell{r0}
        mov [r12], al
        ; call t.34{r0} = isOpen@u8[cell{r1}] -> bool
        call @isOpen@u8
        ; notlog t.33{r0}, t.34{r0}
        or al, al
        sete al
        ; branch t.33{r0}, false, @while_40, @if_51_then
        or al, al
        jz @while_40
        jmp @if_51_then
@no_critical_edge_31:
...
@if_53_end:
        ...
        ; store [memVarAddr{r9}], cell{r0}
        mov [r12], al
        ; call t.38{r0} = isOpen@u8[cell{r1}] -> bool
        call @isOpen@u8
        ; notlog t.37{r0}, t.38{r0}
        or al, al
        sete al
        ; branch t.37{r0}, false, @if_54_end, @if_54_then
        or al, al
        jz @if_54_end
        ; addrof memVarAddr{r9}, cell
        lea r12, [rsp+37]
        ; load cell{r0}, [memVarAddr{r9}]
        mov al, [r12]
        ; move t.39{r3}, cell{r0}
        mov dl, al
        ; addrof memVarAddr{r9}, cell
        lea r12, [rsp+37]
        ; store [memVarAddr{r9}], cell{r0}
        mov [r12], al
        ; or t.39{r3}, t.39{r3}, 2
        or dl, 2
        ; addrof memVarAddr{r9}, curr_r
        lea r12, [rsp+34]
        ; load curr_r{r1}, [memVarAddr{r9}]
        ...
        ; store [memVarAddr{r9}], curr_c{r2}
        mov [r12], si
        ; call setCell@i16@i16@u8[curr_r{r1}, curr_c{r2}, t.39{r3}]
        call @setCell@i16@i16@u8
@if_54_end:
        ...
        mov dil, [r12]
        ; call t.40{r0} = isBomb@u8[cell{r1}] -> bool
        call @isBomb@u8
        ; branch t.40{r0}, true, @if_55_then, @if_55_end
        or al, al
        jnz @if_55_then
        ; addrof memVarAddr{r9}, curr_r
        lea r12, [rsp+34]
...
@if_55_then:
        ...
        ; call printField@i16@i16[curr_r{r1}, curr_c{r2}]
        call @printField@i16@i16
        ; const t.41{r1}, [string-3]
        lea rdi, [string_3]
        ; call printString@@u8[t.41{r1}]
        call @printString@@u8
@main_ret:
        ; restore clobbered non-volatile registers
        pop r12
