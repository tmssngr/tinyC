...
_main:
        ...
        ; const t.space{r8}, 32
        mov bx, 32
        ; addrof a.space{r0}, space
        lea rax, [var_0]
        ; store [a.space{r0}], t.space{r8}
        mov [rax], bx
        ; const t.next{r8}, 63
        mov bx, 63
        ; addrof a.next{r0}, next
        lea rax, [var_1]
        ; store [a.next{r0}], t.next{r8}
        mov [rax], bx
        ; addrof t.ptrToSpace{r8}, space
        lea rbx, [var_0]
        ; addrof a.ptrToSpace{r0}, ptrToSpace
        lea rax, [var_2]
        ; store [a.ptrToSpace{r0}], t.ptrToSpace{r8}
        mov [rax], rbx
        ; end initialize global variables
        ; addrof a.next1{r8}, next
        lea rbx, [var_1]
        ; load t.next1{r1}, [a.next1{r8}]
        mov di, [rbx]
        ; call printIntLf@i16[t.next1{r1}]
        call _printIntLf@i16
        ; addrof a.ptrToSpace1{r8}, ptrToSpace
        lea rbx, [var_2]
        ; load t.ptrToSpace1{r8}, [a.ptrToSpace1{r8}]
        mov rbx, [rbx]
        ; add t.ptrToSpace2{r8}, t.ptrToSpace2{r8}, 2
        add rbx, 2
        ; addrof a.ptrToSpace2{r0}, ptrToSpace
        lea rax, [var_2]
        ; store [a.ptrToSpace2{r0}], t.ptrToSpace2{r8}
        mov [rax], rbx
        ; addrof a.ptrToSpace3{r8}, ptrToSpace
        lea rbx, [var_2]
        ; load t.ptrToSpace3{r8}, [a.ptrToSpace3{r8}]
        mov rbx, [rbx]
        ; load t.0{r1}, [t.ptrToSpace3{r8}]
        mov di, [rbx]
