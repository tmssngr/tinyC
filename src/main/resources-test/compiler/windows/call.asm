...
@main:
        ...
        ; call t.1{r0} = next[] -> u8
        call @next
        ; addrof spillHelper{r7}, t.1
        lea r12, [rsp+56]
        ; store [spillHelper{r7}], t.1{r0}
        mov [r12], al
        ; call t.2{r0} = next[] -> u8
        call @next
        ; addrof spillHelper{r7}, t.2
        lea r12, [rsp+57]
        ; store [spillHelper{r7}], t.2{r0}
        mov [r12], al
        ; call t.3{r0} = next[] -> u8
        call @next
        ; addrof spillHelper{r7}, t.3
        lea r12, [rsp+58]
        ; store [spillHelper{r7}], t.3{r0}
        mov [r12], al
        ; call t.4{r0} = next[] -> u8
        call @next
        ; addrof spillHelper{r7}, arg.0.4
        lea r12, [rsp+32]
        ; store [spillHelper{r7}], t.4{r0}
        mov [r12], al
        ; move t.0{r1}, t.0{r6}
        mov cl, bl
        ; addrof spillHelper{r7}, t.1
        lea r12, [rsp+56]
        ; load t.1{r2}, [spillHelper{r7}]
        mov dl, [r12]
        ; addrof spillHelper{r7}, t.2
        lea r12, [rsp+57]
        ; load t.2{r3}, [spillHelper{r7}]
        mov r8b, [r12]
        ; addrof spillHelper{r7}, t.3
        lea r12, [rsp+58]
        ; load t.3{r4}, [spillHelper{r7}]
        mov r9b, [r12]
...
@doPrint@u8@u8@u8@u8@u8:
        ...
        push r12
        sub rsp, 32
        ; addrof spillHelper{r7}, e
        lea r12, [rsp+96]
        ; load e{r6}, [spillHelper{r7}]
        mov bl, [r12]
        ; addrof spillHelper{r7}, b
        lea r12, [rsp+72]
        ; store [spillHelper{r7}], b{r2}
        mov [r12], dl
        ...
        lea r12, [rsp+88]
        ; store [spillHelper{r7}], d{r4}
        mov [r12], r9b
        ; call printIntLf@u8[a{r1}]
        call @printIntLf@u8
        ; addrof spillHelper{r7}, b
        lea r12, [rsp+72]
        ; load b{r1}, [spillHelper{r7}]
        mov cl, [r12]
        ; call printIntLf@u8[b{r1}]
        call @printIntLf@u8
        ; addrof spillHelper{r7}, c
        lea r12, [rsp+80]
        ; load c{r1}, [spillHelper{r7}]
        mov cl, [r12]
