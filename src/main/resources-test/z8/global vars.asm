...
@main:
        ...
        not implemented
        ; move next, tmp.next{r0}
        not implemented
        ; move ptrToSpace, tmp.ptrToSpace{r10}
        not implemented
        ; call printIntLf[tmp.next{r0}]
        call printIntLf
        ; move tmp.ptrToSpace{r10}, ptrToSpace
        not implemented
        ; cast t.1{r8}(i16), tmp.ptrToSpace{r10}(i16*)
        not implemented
        ; const t.2{r12}, 2
        ld r12, #%00
        ld r13, #%02
        ; add t.0{r8}, t.0{r8}, t.2{r12}
        add r9, r13
        adc r8, r12
        ; cast tmp.ptrToSpace{r10}(i16*), t.0{r8}(i16)
        not implemented
        ; move ptrToSpace, tmp.ptrToSpace{r10}
        not implemented
        ; load t.3{r0}, [tmp.ptrToSpace{r10}]
        lde r0, rr10
        incw r10
        lde r1, rr10
        decw r10
        ; call printIntLf[t.3{r0}]
        call printIntLf
        ; restore globbered non-volatile registers
        pop r13
        pop r12
